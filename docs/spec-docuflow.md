...
- `/settings`
  - 設定画面。AI設定・共有リンク設定・ダッシュボード設定・アカウント削除機能を配置。
...
- 作成 / 更新 / 削除 / お気に入り / ピン / 共有リンク有効化 / 無効化 / アーカイブ / 復元 / 共有リンク閲覧 / アカウント削除の操作を `activity_logs` に保存する。
- `/app` の「最近のアクティビティ」で直近 10 件を表示する。

#### 4.7 ユーザー設定

- `/settings` で以下の設定をカスタマイズできること。
  - **AI 設定**
...
- 設定は `user_settings` テーブルに保存され、各機能でデフォルト値として使用される。

#### 4.8 アカウント削除

- `/settings` からアカウント削除アクションを呼び出せること。
- 対象ユーザーの `documents` / `document_versions` / `user_settings` を削除し、`supabaseAdmin` が有効な場合は Supabase Auth の user も削除する。
